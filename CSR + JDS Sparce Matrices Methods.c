...
__global__ void spmvCSRKernel(float *out, int *matCols, int *matRows,
	...
	if(row<dim){
		...
		{
			prod +=matData[iterator] * vec[matCols[iterator]];
		}
		out[row] += prod; 
...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
	...
	//printf("\nData is: Row: %d colStart: %d cols: %d  rowPerm: %d rows:%d data: %f",row,matColStart[row],matCols[row],matRowPerm[row],matRows[row],matData[row]);
	if(row<dim){
			prod = 0;
			start = 0;
			end = matRows[row];
			//printf("row: %d",matRows[row]);
			for(iterator=start;iterator<end;iterator++)
			{
				prod +=matData[matColStart[iterator]+row] * vec[matCols[matColStart[iterator]+row]];
			}
...
static void spmvCSR(float *out, int *matCols, int *matRows, float *matData,
	...
	dim3 DimBlock(8,1,1);
	//here dim is the number of threads that we should multiply with
